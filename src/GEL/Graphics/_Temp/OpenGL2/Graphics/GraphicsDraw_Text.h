...
#define gelXPrintf(p,c,s,_Format, ...) \
	gelPrintfLeft(p,c,s,_Format, ## __VA_ARGS__)
// - ------------------------------------------------------------------------------------------ - //
...
#define gelXPrintf(p,c,s,_Format,_Args...) \
	gelPrintfLeft(p,c,s,_Format, ## _Args)
// - ------------------------------------------------------------------------------------------ - //
...
inline const float gelTextWidth( const char* Msg, const float Scale = 1.0f ) {
	...
	for ( int CurrentChar = 0; Msg[CurrentChar] != 0; CurrentChar++ ) {
		...
		Pos += Current::Font->PixelWidth[(unsigned char)ch];
	}
...
inline void gelPrint( const Vector2D& _Pos, const ColorType Color, const char* Msg ) {
	...
	for ( int CurrentChar = 0; Msg[CurrentChar] != 0; CurrentChar++ ) {
		...
		if ( ch != 0 ) {
			Vert.AddQuad4( Current::Font->TriStripTable[ Current::Font->BlockWidth[(unsigned char)ch] - 1 ], Pos );
			UV.AddQuad4( &Current::Font->UVStrip[8*(unsigned char)ch] );
		...
		Pos += Vector2D( Current::Font->PixelWidth[(unsigned char)ch], 0 );
...
inline void gelPrint( const Vector2D& _Pos, const ColorType Color, const char* Msg, Real Scale ) {
	...
	for ( int CurrentChar = 0; Msg[CurrentChar] != 0; CurrentChar++ ) {
		...
		if ( ch != 0 ) {
			Vert.AddQuad4( Current::Font->TriStripTable[ Current::Font->BlockWidth[(unsigned char)ch] - 1 ], Pos, Scale );
			UV.AddQuad4( &Current::Font->UVStrip[8*(unsigned char)ch] );
		...
		Pos += Vector2D( Current::Font->PixelWidth[(unsigned char)ch], 0 ) * Scale;
...
// - ------------------------------------------------------------------------------------------ - //
inline void gelPrint( const Vector2D& _Pos, const ColorType Color, const char* Msg, const Matrix2x2& Scale ) {
	//const int _Length = strlen( Msg );
	const int Length = 128;
