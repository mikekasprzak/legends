...
//								SetIntermediateAnimation( Nook_WallJump );
					...
					Velocity.y = -((Real(JumpPower) + Real(0.5)) * Real(0.5));
					if ( Velocity.y < -Real(2) )
						Velocity.y = -Real(2);
					JumpPower--;
				}
			}
...
void GameInit() {	
	...
	TilesetId = gelLoadTileset( "Content/Tileset.png", 8, 8 );
	PlayerId = gelLoadTileset( "Content/Player.png", 16, 16 );
