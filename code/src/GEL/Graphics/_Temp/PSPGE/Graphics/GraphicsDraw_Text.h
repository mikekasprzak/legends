...
// - ------------------------------------------------------------------------------------------ - //
inline void gelSetFont( const Font& FontName ) {
...
inline const float gelTextWidth( const char* Msg, const float Scale = 1.0f ) {
	...
	for ( int CurrentChar = 0; Msg[CurrentChar] != 0; CurrentChar++ ) {
		...
		Pos += Current::Font->PixelWidth[ch];
	}
...
inline void gelPrint( const Vector2D& _Pos, const ColorType Color, const char* Msg ) {
	...
	for ( int CurrentChar = 0; Msg[CurrentChar] != 0; CurrentChar++ ) {
		...
		if ( ch != 0 ) {
			Vert.AddQuad4( Current::Font->TriStripTable[ Current::Font->BlockWidth[ch] - 1 ], Pos );
			UV.AddQuad4( &Current::Font->UVStrip[8*ch] );
		...
		Pos += Vector2D( Current::Font->PixelWidth[ch], 0 );
...
inline void gelPrint( const Vector2D& _Pos, const ColorType Color, const char* Msg, Real Scale ) {
	...
	for ( int CurrentChar = 0; Msg[CurrentChar] != 0; CurrentChar++ ) {
		...
		if ( ch != 0 ) {
			Vert.AddQuad4( Current::Font->TriStripTable[ Current::Font->BlockWidth[ch] - 1 ], Pos, Scale );
			UV.AddQuad4( &Current::Font->UVStrip[8*ch] );
		...
		Pos += Vector2D( Current::Font->PixelWidth[ch], 0 ) * Scale;
