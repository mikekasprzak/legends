// - ------------------------------------------------------------------------------------------ - //
#ifndef __Graphics_Support_SDL_H__
#define __Graphics_Support_SDL_H__
// - ------------------------------------------------------------------------------------------ - //
#ifdef USES_SDL
// - ------------------------------------------------------------------------------------------ - //
...
extern SDL_GLES_Context* GLESContext;
...
#ifdef USES_OPENGLES2
	SDL_GLES_DeleteContext( GLESContext );
...
inline void gelsInitGraphicsAPI() {
	// Initialize SDL //
...
inline void gelsLogGraphicsDetails() {
...
inline void gelsAutodetectScreenShape() {
	// Get information about our video hardware //    
	const SDL_VideoInfo* VideoInfo = SDL_GetVideoInfo();
...
//		SDL_GL_SetAttribute( SDL_GL_BUFFER_SIZE, 32 );
...
#ifdef USES_MAEMO
		extern void Maemo_SetCaption( const char* Text );
		Maemo_SetCaption( ProductName );
#else
		...
		// Create our Screen //
#ifdef USES_OPENGLES2
		Buffer = SDL_SetVideoMode( ActualScreen::Width, ActualScreen::Height, ColorDepth, SDL_SWSURFACE | SDL_FULLSCREEN );	
#else
		...
		GLESContext = SDL_GLES_CreateContext();
		Log( "**\n" );
		SDL_GLES_MakeCurrent( GLESContext );
		Log( "**\n" );

		SDL_GLES_SetVideoMode();
		Log( "**\n" );
#else
...
inline void gelsSetSwapInterval( const int Var = 1 ) {
...
#endif // USES_SDL //
// - ------------------------------------------------------------------------------------------ - //
#endif // __Graphics_Support_SDL_H__ //
// - ------------------------------------------------------------------------------------------ - //
